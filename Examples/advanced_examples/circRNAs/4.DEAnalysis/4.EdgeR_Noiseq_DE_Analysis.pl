...
BEGIN{
	...
	$dir="../3.circRNAs_identification/circRNAs_results/"; #Path of the directory with the tab files
	$projectdir="."; #Path of the directory where the output files will be saved.
	$targetfile="../../../basic_examples/circRNAs/data/targets.txt"; #Path of the target file.
	$contrastfile="../../../basic_examples/circRNAs/data/contrast.txt"; #Path of the contrast file.
	...
	$cpmvalue=0; #Cutoff for the counts per million value to be used in filter processing with methods 1 and 3 with Noiseq software and in filter processing with EdgeR (1 cpm by default).
	...
	$filter="no"; #This value refers to filter processing in the reads (Should be "yes" or "no").
	...
	$repthreshold=1; #Number of replicates that have to contains at least a defined number of reads per million to perform the filter with EdgeR software(2 replicates by default)
	$label="ciRNAs";
}
